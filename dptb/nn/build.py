...
class DeterministicExpertSeed:
    ...
    def __enter__(self):
        ...
        if torch.cuda.is_available(): self.torch_cuda_state = torch.cuda.get_rng_state_all()
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if torch.cuda.is_available(): torch.cuda.set_rng_state_all(self.torch_cuda_state)
...
class DistanceEnsembleWrapper(nn.Module):
    ...
    def __getattr__(self, name):
        try:
            ...
            if experts is not None and len(experts) > 0 and hasattr(experts[0], name):
                return getattr(experts[0], name)
            raise e

    def _build_excluded_stitch_keys(self):
        keys = {"expert_idx", "expert_edge_mask", "expert_node_mask", "__slices__", "__cumsum__", "__cat_dims__",
                "__num_nodes_list__", "__data_class__", "edge_vec", "edge_index", "edge_type", "atom_types", "batch",
                "ptr", "cell", "pbc", "pos", "positions", "edge_lengths"}
        for const_name in ["ATOM_TYPE_KEY", "EDGE_TYPE_KEY", "EDGE_INDEX_KEY", "EDGE_VEC_KEY", "POSITIONS_KEY",
                           "BATCH_KEY", "CELL_KEY", "PBC_KEY"]:
    ...
    def _build_expert_masks(self, batch, expert_idx):
        ...
        edge_mask = (dist >= d_min) if expert_idx == self.num_experts - 1 else (dist >= d_min) & (dist < d_max)

        num_nodes = batch.get(getattr(AtomicDataDict, "ATOM_TYPE_KEY", "atom_types"),
                              batch.get(getattr(AtomicDataDict, "POSITIONS_KEY", "pos"), dist)).shape[0]
        node_mask = torch.ones(num_nodes, dtype=torch.bool, device=dist.device)
        if d_min > 0: node_mask.fill_(False)
    ...
    def _stitch_edge_aligned_outputs(self, res, res_i, mask):
        ...
        for key, src in res_i.items():
            ...
            dst = res[key]
            if not torch.is_tensor(src) or not torch.is_tensor(
                dst) or src.ndim == 0 or dst.ndim == 0 or src.shape != dst.shape or src.shape[0] != num_edges: continue
            dst[mask] = src[mask]

    def forward(self, batch):
        if "edge_lengths" not in batch: batch = with_edge_vectors(batch, with_lengths=True)
        expert_idx = batch.get("expert_idx", None)

        if expert_idx is not None:
            expert_idx_val = int(expert_idx.detach().item()) if torch.is_tensor(expert_idx) else int(expert_idx)
            ...
            if "expert_edge_mask" not in clean_batch or "expert_node_mask" not in clean_batch:
                clean_batch["expert_edge_mask"], clean_batch["expert_node_mask"] = self._build_expert_masks(clean_batch,
                                                                                                            expert_idx_val)
            return self.experts[expert_idx_val](clean_batch)

        base_batch = batch.copy()
        batch_0 = base_batch.copy()
        batch_0["expert_edge_mask"], batch_0["expert_node_mask"] = self._build_expert_masks(base_batch, 0)
        res = self.experts[0](batch_0)

        for i in range(1, self.num_experts):
            edge_mask_i, node_mask_i = self._build_expert_masks(base_batch, i)
            if not bool(edge_mask_i.any().item()): continue
            batch_i = base_batch.copy()
            batch_i["expert_edge_mask"], batch_i["expert_node_mask"] = edge_mask_i, node_mask_i
            res_i = self.experts[i](batch_i)
            self._stitch_edge_aligned_outputs(res, res_i, edge_mask_i)

        for k in ["expert_edge_mask", "expert_node_mask"]:
...
def _construct_single_model(init_nnenv, init_nnsk, init_mixed, init_dftbsk, model_options, common_options,
    ...
    if num_xgrid is not None and (init_dftbsk or (init_mixed and model_options.get("dftbsk") is not None)):
...
def _build_ensemble_from_wrapper_state(wrapper_state_dict, distance_ranges, init_nnenv, init_nnsk, init_mixed,
    ...
    if _count_experts_in_state_dict(wrapper_state_dict) != len(distance_ranges):
...
def build_model(checkpoint: str = None, model_options: dict = {}, common_options: dict = {}, train_options: dict = {},
                no_check: bool = False, device: str = None):
    from_scratch = checkpoint is None
    init_nnenv = init_nnsk = init_mixed = init_dftbsk = False
    ...
    if not from_scratch:
        if checkpoint.split(".")[-1] == "json":
            ...
            ckptconfig, ckpt_state_dict = f['config'], f.get("model_state_dict", None)
            del f
        if not model_options: model_options = ckptconfig["model_options"]
        if not common_options: common_options = ckptconfig["common_options"]
        if not train_options: train_options = ckptconfig.get("train_options", {})

    if model_options.get("dftbsk"):
        if model_options.get("embedding") and model_options.get("prediction"):
            init_mixed = True
        else:
            init_dftbsk = True
    ...
    if device: common_options.update({"device": device})
    ...
    if distance_ranges is not None and len(distance_ranges) > 1:
        ...
        if from_scratch or (not from_scratch and ckpt_state_dict is None):
        ...
        elif _count_experts_in_state_dict(ckpt_state_dict) > 0:
            model = _build_ensemble_from_wrapper_state(ckpt_state_dict, distance_ranges, init_nnenv, init_nnsk,
                                                       init_mixed, init_dftbsk, model_options, common_options)
        else:
            with DeterministicExpertSeed(1):
                prototype_model = _construct_single_model_from_reference(checkpoint, init_nnenv, init_nnsk, init_mixed,
                                                                         init_dftbsk, model_options, common_options)
            model = _replicate_prototype_to_ensemble(prototype_model, distance_ranges, init_nnenv, init_nnsk,
                                                     init_mixed, init_dftbsk, model_options, common_options)
    else:
        with DeterministicExpertSeed(1):
            if from_scratch:
                model = _construct_single_model(init_nnenv, init_nnsk, init_mixed, init_dftbsk, model_options,
                                                common_options)
            else:
                model = _construct_single_model_from_reference(checkpoint, init_nnenv, init_nnsk, init_mixed,
                                                               init_dftbsk, model_options, common_options)
    ...
    if isinstance(expected_value, dict):
        for subk, subv in expected_value.items():
            if subk not in target_dict.get(base_key, {}):
                ...
                deep_dict_difference(f"{subk}", subv, copy.deepcopy(target_dict[base_key]))
