...
import argparse, hashlib, json, math, os, signal, subprocess, sys, threading, time, uuid
from lifecycle import register_worker, run_registered
...
def shared_contract():
    ...
    from h0rebuild.numerical_identity import execution
    ...
    return {'schema':6,'files':files,'execution_environment':execution()['identity'],'Hmax_eV':.005,'Smax':1e-6,
...
def elapsed_charge(delta,stopped):
    return 0. if stopped else delta
...
def wait_active(child,budget):
    ...
    while child.poll() is None:
        ...
        if spent>=budget:
            if child.poll() is not None:return child.returncode,False,spent,excluded
            try:os.killpg(child.pid,signal.SIGTERM)
            except ProcessLookupError:
                child.wait();return child.returncode,False,spent,excluded
...
def run_worker(cid,attempt_dir,ready_fd,start_fd):
    register_worker(attempt_dir,ready_fd,start_fd)
...
def run_attempt(cid,gpu,run,contract,budget,wall_budget=86400.):
    import fcntl
    if not all(math.isfinite(x) and x>0 for x in (budget,wall_budget)):
        raise ValueError('Budgets must be finite and positive')
    lock_dir=Path(os.environ.get('H0_CASE_LOCK_DIR',str(ROOT/'case_locks')))
    lock_dir.mkdir(parents=True,exist_ok=True)
    with (lock_dir/(digest({'case':cid,'identity':digest(contract)})+'.lock')).open('a') as case_lock:
        try:fcntl.flock(case_lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
        except BlockingIOError:raise RuntimeError(f'An existing dispatcher/worker owns case {cid}; reconcile before retry')
        return _run_attempt_locked(cid,gpu,run,contract,budget,wall_budget,case_lock)

def _run_attempt_locked(cid,gpu,run,contract,budget,wall_budget,case_lock):
    import fcntl
    ...
    if committed.exists():
        ...
        if (accept_receipt(row,cid,ref['attempt_id'],identity,execution['returncode'])
                and not execution['timed_out'] and execution.get('registered') is True):
            return {'id':cid,'status':row['status'],'attempt_id':ref['attempt_id'],'reused':True}
    for p in list(case_dir.glob('*/attempt.json'))+list(case_dir.glob('*/worker.json')):
    ...
    with (lock_dir/f'gpu{gpu}.lock').open('a') as lock:
        ...
        execution=run_registered([sys.executable,str(Path(__file__).resolve()),'--worker',cid,'--attempt-dir',str(p)],
            p,{**os.environ,'CUDA_VISIBLE_DEVICES':str(gpu)},(lock.fileno(),case_lock.fileno()),budget,wall_budget)
        rc=execution['returncode'];timeout=execution['timed_out']
...
def main():
    ...
    parser.add_argument('--ready-fd',type=int);parser.add_argument('--start-fd',type=int)
    parser.add_argument('--ids',nargs='+');parser.add_argument('--budget',type=float,default=1800.)
    parser.add_argument('--wall-budget',type=float,default=86400.)
    parser.add_argument('--field-work-mb',type=float,default=None)
    parser.add_argument('--structure-factor-work-mb',type=float,default=None)
    parser.add_argument('--projector-reuse-mb',type=float,default=None)
    parser.add_argument('--gpus',nargs='+',type=int,default=[0,1]);args=parser.parse_args()
    if args.field_work_mb is not None:os.environ['H0_FIELD_MAX_WORK_MB']=str(args.field_work_mb)
    if args.structure_factor_work_mb is not None:os.environ['H0_STRUCTURE_FACTOR_MAX_WORK_MB']=str(args.structure_factor_work_mb)
    if args.projector_reuse_mb is not None:os.environ['H0_PROJECTOR_REUSE_MAX_MB']=str(args.projector_reuse_mb)
    from runtime_options import memory_budgets
    memory_budgets()  # Validate before taking locks or starting children.
    if args.worker:
        if args.ready_fd is None or args.start_fd is None:raise ValueError('Worker requires registered parent handshake')
        return run_worker(args.worker,args.attempt_dir,args.ready_fd,args.start_fd)
    import fcntl
    ...
    run=ROOT/'acceptance_v5'/digest(shared);run.mkdir(parents=True,exist_ok=True)
    with (run/'dispatcher.lock').open('a') as lock:
        fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB);write(run/'contract.json',shared)
        from h0rebuild.numerical_identity import execution
        write(run/'environment_provenance.json',execution())
        ...
        write(ROOT/'acceptance_v5/current.json',{'run':str(run)});guard=threading.Lock()
        def lane(gpu,cohort):
            for cid in cohort:
                try:row=run_attempt(cid,gpu,run,case_contract(cid,shared,cat),args.budget,args.wall_budget)
