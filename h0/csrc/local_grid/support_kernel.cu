...
std::vector<torch::Tensor> build_atom_support_cuda(
    ...
    vector3(center,device,torch::kFloat64);finite(center);
    vector3(nmin,device,torch::kInt64);coordinates(nmin);
    vector3(counts,device,torch::kInt32);vector3(shape,device,torch::kInt64);
    TORCH_CHECK((counts>=0).all().item<bool>() && (shape>0).all().item<bool>(), "invalid grid counts or shape");
    tensor(cell,device,torch::kFloat64,2);finite(cell);
    TORCH_CHECK(cell.size(0)==3 && cell.size(1)==3, "cell must be 3x3");
    TORCH_CHECK(std::isfinite(dr) && dr>0 && std::isfinite(rcut) && rcut>0 && std::isfinite(rcut*rcut), "invalid radial grid");
    TORCH_CHECK(num_channels>0 && num_channels<=16 && n_intervals>0 && norb>0, "unsupported channels or orbital counts");
    tensor(spline_coeffs,device,torch::kFloat64,3);finite(spline_coeffs);
    TORCH_CHECK(spline_coeffs.size(0)==num_channels && spline_coeffs.size(1)==4 && spline_coeffs.size(2)==n_intervals, "invalid spline shape");
    tensor(descriptors,device,torch::kInt32,2);
    TORCH_CHECK(descriptors.size(0)==norb && descriptors.size(1)==3, "invalid descriptor shape");
    auto channels=descriptors.select(1,0);auto ls=descriptors.select(1,1);auto ms=descriptors.select(1,2);
    TORCH_CHECK(((channels>=0)&(channels<num_channels)).all().item<bool>(), "descriptor channel out of range");
    TORCH_CHECK(((ls>=0)&(ls<=4)).all().item<bool>() && ((ms>=-ls)&(ms<=ls)).all().item<bool>(), "invalid harmonic descriptor");
    tensor(field_values,device,torch::kFloat64,3);finite(field_values);
    tensor(spin_z_values,device,torch::kFloat64,has_spin_z?3:1);
    if(has_spin_z) {TORCH_CHECK(spin_z_values.sizes()==field_values.sizes(), "spin field shape mismatch");finite(spin_z_values);}
    else TORCH_CHECK(spin_z_values.numel()==0, "unused spin field must be empty");
    ...
    for(int a=0;a<3;++a) TORCH_CHECK(shape_a[a]==field_values.size(a), "field grid shape mismatch");
    int64_t total_candidates = product(product(counts_a[0],counts_a[1]),counts_a[2]);
    ...
    if(total_candidates>0) find_active_nodes_kernel<<<blocks, threads, 0, stream>>>(
        (int)total_candidates,
    ...
    // Candidate-box volume can be much larger than the spherical AO support
    // in an oblique cell. Check actual output dimensions after compaction,
    // before allocating any AO/coordinate output or launching its evaluator.
    support_capacity(active_indices.size(0),norb);
    ...
    if (npoints > 0) {
        ...
        product(product(box0,box1),box2);
