# =============================================================================
# FROZEN TEST-ONLY REFERENCE -- NEVER EDIT.
#
# This module is a verbatim vendor of dptb/data/dataset/lmdb_dataset.py at git
# commit 85f87bb ("fix: resolve 3 audit findings on the integrated waves"),
# i.e. the last commit BEFORE PR-H (90a48ba) decomposed the monolithic
# ``LMDBDataset.get()`` into dptb/data/dataset/record_pipeline.py.  It serves
# as the differential oracle for test_lmdb_differential_oracle.py: the live,
# refactored loader must reproduce this ``get()`` byte for byte.
#
# The ONLY deviations from the committed file are the mechanical minimum needed
# to import it alongside the live code:
#   * the class is renamed LMDBDataset -> OracleLMDBDataset (incl. its two
#     internal self-references in ``__init__``);
#   * the two package-relative imports (``..transforms`` / ``._base_datasets``)
#     are rewritten as absolute imports, because this copy lives under
#     dptb/tests/ instead of dptb/data/dataset/.
# Semantics are untouched.  Do not "fix", reformat, or modernize this file.
# =============================================================================
import numpy as np
...
from dptb.data.dataset._base_datasets import (
    AtomicDataset,
...
class OracleLMDBDataset(AtomicDataset):
    ...
    def __init__(
        ...
        if class_type != OracleLMDBDataset:
            if "download" not in self.__class__.__dict__:
                class_type.download = OracleLMDBDataset.download
