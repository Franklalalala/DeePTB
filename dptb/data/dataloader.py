    ...
    def _ordered_indices(self) -> List[int]:
        ...
        if self.bucket_size > 1:
            ...
            for start in range(0, len(indices), self.bucket_size):
                ...
                chunk.sort(key=self._cost, reverse=True)
                out.extend(chunk)
            indices = out
        return indices
        ...
        for idx in self._ordered_indices():
            ...
            if cost > self.max_cost:
                ...
                if self.drop_oversized:
                    if idx not in self._warned_oversized:
                        log.warning(
                            "drop oversized dynamic-batch sample idx=%s cost=%s > max_cost=%s",
                            idx,
                            cost,
                            self.max_cost,
                        )
                ...
                if idx not in self._warned_oversized:
                    log.warning(
                        "yield oversized singleton dynamic-batch sample idx=%s cost=%s > max_cost=%s",
                        idx,
                        cost,
                        self.max_cost,
                    )
...
def resolve_dynamic_batch_options(
    ...
    *,
    ...
    if opts.get("max_cost") is None:
        ...
        q = float(opts.get("calibration_quantile", 0.90))
        q = min(max(q, 0.0), 1.0)
        ...
        log.info(
            "dynamic_batch calibration: mode=%s batches=%s quantile=%.3f max_cost=%s max_samples=%s",
            opts.get("mode"),
            len(batch_costs),
            q,
            opts["max_cost"],
