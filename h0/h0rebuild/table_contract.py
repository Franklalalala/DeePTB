...
FILES=('pyabacus_integrals.py','models.py','nonlocal_kb.py','harmonics.py','upf.py','orb.py',
       'radial_quadrature.py','radial.py','cuda_two_center.py','scalar_upf.py','offline.py','table_contract.py','numerical_identity.py')

def current():
    root=Path(__file__).parent
    from .numerical_identity import capture
    # Load the native generator, including its selected BLAS dependencies.
    from pyabacus import ModuleNAO, ModuleBase
    return {'sources':{name:sha256(root/name) for name in FILES},
            'generator_environment':capture(('numpy','scipy','torch','pyabacus'))['identity']}
