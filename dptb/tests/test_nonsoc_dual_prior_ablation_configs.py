...
def _remove_memory_axis(embedding: dict) -> None:
    embedding["use_soft_edge_memory"] = "<memory-axis>"
    for key in list(embedding):
        if key.startswith("soft_edge_memory_") and key != "soft_edge_memory_diagnostics_mode":
            embedding.pop(key)
...
def _canonical_without_axes(config: dict) -> dict:
    ...
    prediction["add_prior"] = "<head-axis>"
    for key in (
    ...
    for name, config in configs.items():
        ...
        axes.add(
            (
                ...
                "residual_add_prior" if prediction["add_prior"] else "direct_full_h",
                embedding["use_soft_edge_memory"],
            )
        )
...
def test_axis_contracts_and_all_other_options_are_invariant(tmp_path: Path) -> None:
    ...
    for config in configs.values():
        ...
        add_prior = prediction["add_prior"]
