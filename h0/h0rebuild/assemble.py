...
def assemble_h0(
    ...
    *,
    ...
    projector_reuse_max_mb: float = 0.0,
    projector_search: str = 'midpoint',
    cuda_pair_batch_size: int = 128,
    ...
    """
    if projector_search not in ('midpoint', 'anchor'):
        raise ValueError('projector_search must be midpoint or anchor')
    if isinstance(cuda_pair_batch_size, bool) or not isinstance(cuda_pair_batch_size, int) or cuda_pair_batch_size < 1:
    ...
    if not np.isfinite(projector_reuse_max_mb) or projector_reuse_max_mb < 0:
        raise ValueError('projector_reuse_max_mb must be finite and non-negative')
    if projector_reuse_max_mb and not cuda_precompiled:
        raise ValueError('projector reuse requires the precompiled CUDA two-center backend')
    two_center = None
    if two_center_backend == 'pyabacus':
        if cuda_precompiled:
            if offline_table_dir is not None:
                # Each prepared loader enforces the numerical contract at its
                # own entry. Avoid scanning the same environment twice here.
                from .offline import prepared_two_center
                from .shared_radial_store import is_shared_store, prepared_shared_two_center
                if is_shared_store(offline_table_dir):
                    prepared_two_center = prepared_shared_two_center
    ...
    if projector_reuse_max_mb:
        from .projector_reuse import ProjectorReuseTwoCenter
        two_center = ProjectorReuseTwoCenter(two_center, max_mb=projector_reuse_max_mb)
    ...
    if local_mode == 'fft_grid_periodic':
        ...
        periodic_cache = PeriodicFFTGridAOCache(field,
            ...
            **({'spin_z_field':spin_z_field} if cuda_precompiled else {}))
        home_positions=structure.cart_positions
        if spin_z_field is not None and not cuda_precompiled:
    ...
    pair_stream = iter_pair_images(
        structure, orbital_cutoffs, index=atom_index, extra_cutoff_bohr=extra_cutoff
    )
    if cuda_precompiled:
        from .pair_batches import scalar_pair_batches, with_local_chunks
        atom_species=[a.species for a in structure.atoms]
        evaluated_pairs = scalar_pair_batches(two_center, pair_stream,
            atom_species, cuda_pair_batch_size)
        evaluated_pairs = with_local_chunks(evaluated_pairs,periodic_cache,
            atom_species,evaluators,home_positions,cuda_pair_batch_size)
    else:
        evaluated_pairs = ((edge,None,None,None,None) for edge in pair_stream)
    for (i, j, R, ci, cj), batch_s, batch_t, batch_v, batch_z in evaluated_pairs:
        ...
        if two_center is None:
            ...
            s,t = ((batch_s,batch_t) if cuda_precompiled else
                two_center.scalar_pair(structure.atoms[i].species,structure.atoms[j].species,ci,cj))
            if local_mode == 'fft_grid_periodic':
                v = batch_v if cuda_precompiled else periodic_cache.contract_pair(eval_i,eval_j,ci,home_positions[j],R)
        ...
        if spin_z_field is not None:
            vz=batch_z if cuda_precompiled else spin_z_cache.contract_pair(eval_i,eval_j,ci,home_positions[j],R)
            ni,nj=v.shape
            v_spin[:ni,:nj]+=vz
            v_spin[ni:,nj:]-=vz
        if include_nonlocal:
            ...
            if atom_index is not None:
                if projector_search == 'anchor':
                    # Every contributing k satisfies |ci-k| <= r_i + r_proj(k).
                    # ci is fixed for all outgoing pairs of i. Retain only one
                    # atom's ordered superset; the native wrapper still applies
                    # both original per-species cutoffs in the original order.
                    if anchor_atom != i:
                        from .projector_candidates import anchor_candidates as query_anchor
                        anchor_candidates = query_anchor(atom_index, ci, eval_i.basis.rcut, max_projector_cutoff)
                        if projector_reuse_max_mb:
                            from .projector_candidates import PreparedProjectorCandidates
                            anchor_candidates = PreparedProjectorCandidates(
                                anchor_candidates, structure, two_center, ci, eval_i.basis.rcut)
                        anchor_atom = i
                    candidates = anchor_candidates
                else:
                    radius = max_projector_cutoff + max(eval_i.basis.rcut, eval_j.basis.rcut) + 0.5 * np.linalg.norm(ci - cj)
                    candidates = ((hit.atom_index, hit.center_bohr)
    ...
    result = AssemblyResult(
        ...
        metadata={
            ...
            "spin_z_support_shared": bool(cuda_precompiled and spin_z_field is not None),
            "two_center_backend": 'cuda_precompiled' if cuda_precompiled else two_center_backend,
            "cuda_precompiled": bool(cuda_precompiled),
            "cuda_pair_batch_size": cuda_pair_batch_size if cuda_precompiled else None,
            "two_center_metadata": ({**two_center.metadata,**getattr(two_center, 'timing', getattr(two_center, 'stats', {}))} if two_center is not None else None),
            ...
            "projector_search": projector_search,
