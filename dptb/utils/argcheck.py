...
def slem():
    ...
    doc_so2_fusion_mode = "SO2_Linear fusion mode. Supported: `staged`, `streamed_m_major_ref`, `streamed_m_major_aggressive`, `streamed_m_major_cueq`, `streamed_m_major_triton_fused`, `streamed_m_major_triton_grouped`. Default keeps the current staged path; `streamed_m_major_cueq` uses the grouped m-major SO2 dataflow and should be paired with `mole_linear_mode=cueq_indexed_linear` for the cuEquivariance indexed-linear backend. `streamed_m_major_triton_fused` is an experimental per-group Triton bridge. `streamed_m_major_triton_grouped` is an experimental grouped-persistent Triton bridge that launches once per m for active l groups when fp32 CUDA Triton is available and otherwise uses a torch fallback."
    ...
    doc_mole_linear_m0_mode = "Optional override for the scalar m=0 MoLELinear backend inside SO2_Linear. Supported: `split_loop`, `indexed_ref`, `cueq_indexed_linear`. Default `None` inherits `mole_linear_mode` or uses the DPTB_MOLE_LINEAR_M0_MODE environment variable."
