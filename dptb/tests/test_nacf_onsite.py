"""Fused batched onsite XC: neighbourhoods, spline boundary rules, packed density banks, environment-probe
reductions, and CUDA parity of the fused kernels against the reference engine. Native onsite neighbours live in
test_nacf_topology_native.py; CUDA cases that evaluate without autograd need the verified NACF prebuilt binary."""
import math
...
from dptb.nacf.density import density_sum_cuda, probe_environment_density
from dptb.nacf.onsite import (PRUNE_MARGIN_BOHR, OnsiteXCEvaluator, PackedDensityBank, SplineDensity, fused_onsite_blocks,
                              fused_onsite_density, onsite_candidates, onsite_neighbor_lists, pz81_potential,
                              reference_onsite_blocks)
from dptb.tests._requires import requires_nacf_prebuilt
from dptb.tests.nacf_support import synthetic_density
...
def test_fused_density_rejects_missing_neighbours_dtype_and_unknown_species_on_cuda():
...
def test_probe_density_weighted_empty_and_endpoint_contract():
    ...
    rho = (torch.exp(-torch.linalg.vector_norm(p[0] - torch.tensor(neighbors[0]), dim=-1))
          + 2 * torch.exp(-torch.linalg.vector_norm(p[0] - torch.tensor(neighbors[1]), dim=-1)))
...
def test_fused_density_channel_clipping_support_and_empty():
    ...
    for start in range(0, 37, 16):
        ...
        rr = r.clamp(knots[0], knots[-1])
        idx = (torch.searchsorted(knots, rr.contiguous(), right=True) - 1).clamp(0, len(knots) - 2)
        d = rr - knots[idx]
        value = torch.zeros_like(r)
        for c in coeff:
            value += (((c[0, idx] * d + c[1, idx]) * d + c[2, idx]) * d + c[3, idx]).clamp_min(0)
