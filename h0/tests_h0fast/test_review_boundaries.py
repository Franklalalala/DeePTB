...
def test_receipt_and_pause_contract():
    ...
    assert elapsed_charge(86400,False)==86400 and elapsed_charge(.25,True)==0
...
def test_real_stopped_worker_does_not_exhaust_budget():
    ...
    child=subprocess.Popen([sys.executable,'-S','-c','import time; time.sleep(.1)'],start_new_session=True)
    try:
