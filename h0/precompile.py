...
def build(name, force=False, check=False):
    ...
    ldflags = []
    if lane == 'two_center':
        from pyabacus import ModuleNAO
        if not os.environ.get('ABACUS_SOURCE_DIR'):
            raise RuntimeError('Set ABACUS_SOURCE_DIR to the source/ directory matching the installed pyabacus ABI')
        upstream = Path(os.environ['ABACUS_SOURCE_DIR']).resolve()
        nao = Path(ModuleNAO.__file__).resolve().parent
        if not (upstream/'source_base').is_dir():
            raise RuntimeError('ABACUS_SOURCE_DIR must contain source_base/')
        includes += [str(upstream), str(upstream/'source_base'), str(upstream/'source_base/module_container'), str(Path(sys.prefix)/'include')]
        includes += [p for p in os.environ.get('H0_EXTRA_INCLUDE_DIRS', '').split(os.pathsep) if p]
        for p in sorted(nao.glob('*.so*')):
            if p.is_file():
                deps[str(p)] = sha256(p)
        ldflags += [f'-L{nao}', f'-Wl,-rpath,{nao}', '-lnaopack', '-lopenblas', f'-L{Path(sys.prefix)/"lib"}']
