...
def private_copy(state):
    """Own the result and track every source storage read on a CUDA side stream.

    A producer event orders reads; record_stream separately delays allocator
    reuse when a private master is evicted before its asynchronous copy ends.
    """
    import torch
    def record(value):
        if isinstance(value, torch.Tensor) and value.is_cuda:
            ...
            for k, v in value.items(): record(k); record(v)
        elif isinstance(value, (list, tuple)):
            for v in value: record(v)
        elif dataclasses.is_dataclass(value):
            for f in dataclasses.fields(value): record(getattr(value, f.name))
    record(state)
...
def _prepared_two_center(species_data, *, store, dr_bohr, nspin, device, prepare):
    ...
    with torch.cuda.device(dev):
        obj.__dict__.update(private_copy(state))
    obj.device=dev;obj.sd=sd
