...
void check_tensor(const torch::Tensor& t, const torch::Tensor& anchor, torch::ScalarType dtype, int dim) {
    ...
    TORCH_CHECK(t.numel() <= INT32_MAX - 256, "tensor exceeds native int32 indexing capacity");
    for(auto size:t.sizes()) TORCH_CHECK(size<=INT32_MAX-256, "dimension exceeds native int32 indexing capacity");
}
...
void check_layout(const torch::Tensor& anchor, const torch::Tensor& ls, const torch::Tensor& zs,
    ...
    TORCH_CHECK(((ms>=-ls)&(ms<=ls)).all().item<bool>(), "invalid harmonic m");
    check_range(offsets,0,ls.numel()+1);
...
std::pair<torch::Tensor, torch::Tensor> eval_two_center_batch(
    ...
    TORCH_CHECK(int64_t(max_norb)*max_norb <= INT32_MAX-256 &&
        (n_pairs==0 || int64_t(max_norb)*max_norb <= (INT32_MAX-256)/n_pairs), "output exceeds native int32 indexing capacity");
...
torch::Tensor eval_projector_overlap_batch(
    ...
    int n_pairs = displacements.size(0);
    TORCH_CHECK(int64_t(max_nproj)*max_norb <= INT32_MAX-256 &&
        (n_pairs==0 || int64_t(max_nproj)*max_norb <= (INT32_MAX-256)/n_pairs), "output exceeds native int32 indexing capacity");
...
torch::Tensor assemble_nonlocal_candidates(
    ...
    TORCH_CHECK(norb_i <= (INT32_MAX-256)/mult/mult/norb_j, "output exceeds native int32 indexing capacity");
