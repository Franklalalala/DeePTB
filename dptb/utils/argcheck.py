...
def slem():
    ...
    doc_mole_linear_mode = "MoLELinear backend. Supported: `split_loop`, `indexed_ref`, `cueq_indexed_linear`, `triton_grouped_linear`, `triton_fused_expert_linear`. Default `None` uses the DPTB_MOLE_LINEAR_MODE environment variable or falls back to `split_loop`. `triton_grouped_linear` uses a Triton persistent grouped GEMM backend after materializing mixed weights. `triton_fused_expert_linear` is an experimental full-expert path that mixes expert weights inside the grouped Triton tile and falls back to split-loop formulas when Triton is unavailable unless DPTB_TRITON_LINEAR_REQUIRE=1."
    doc_mole_linear_m0_mode = "Optional override for the scalar m=0 MoLELinear backend inside SO2_Linear. Supported: `split_loop`, `indexed_ref`, `cueq_indexed_linear`, `triton_grouped_linear`, `triton_fused_expert_linear`. Default `None` inherits `mole_linear_mode` or uses the DPTB_MOLE_LINEAR_M0_MODE environment variable."
