...
def prepare(raw, store, ids=None):
    ...
    for folder in sorted(raw.iterdir()):
        ...
        if requested_ids is not None and folder.name not in requested_ids: continue
        try:
...
if __name__ == '__main__':
    p=argparse.ArgumentParser();p.add_argument('raw');p.add_argument('store');p.add_argument('--ids',nargs='+')
    a=p.parse_args();prepare(a.raw,a.store,a.ids)
