...
def _manifest_code_identity_sha256(source: Any) -> str:
    """Validate and hash the Gate-1/builder identity declared by a manifest."""

    if not isinstance(source, Mapping):
        raise ValueError("P2 manifest lacks a Gate-1/builder code identity mapping")
    if source.get("code_identity_schema") != P2_TABLE_CODE_IDENTITY_SCHEMA:
        raise ValueError(
            "P2 manifest Gate-1/builder code identity has the wrong schema"
        )
    canonical = {
        "schema": P2_TABLE_CODE_IDENTITY_SCHEMA,
        "gate1_script_sha256": _normalized_sha256(
            source.get("gate1_script_sha256"),
            label="source.gate1_script_sha256",
        ),
        "builder_script_sha256": _normalized_sha256(
            source.get("builder_script_sha256"),
            label="source.builder_script_sha256",
        ),
    }
    encoded = json.dumps(
    ...
    declared = _normalized_sha256(
        source.get("code_identity_sha256"),
        label="source.code_identity_sha256",
    )
    if declared != derived:
        raise ValueError(
            "P2 manifest code_identity_sha256 is inconsistent with its "
            "Gate-1/builder hashes"
        )
    return derived
...
class P2TableStore:
    ...
    def __init__(
        ...
        *,
        ...
        metadata_groups = (
            ...
            ("projector", self.projector_tables),
        )
        ...
        source_declares_code_identity = isinstance(source, Mapping) and any(
            field in source for field in source_identity_fields
        )
        shard_declares_code_identity = any(
            "code_identity_sha256" in metadata
            for _, entries in metadata_groups
            for metadata in entries.values()
        )
        if source_declares_code_identity or shard_declares_code_identity:
            self.table_code_identity_sha256 = _manifest_code_identity_sha256(source)
            for category, entries in metadata_groups:
                for key, metadata in entries.items():
                    shard_identity = _normalized_sha256(
                        metadata.get("code_identity_sha256"),
                        label=f"{category}:{key}.code_identity_sha256",
                    )
                    if shard_identity != self.table_code_identity_sha256:
                        raise ValueError(
                            f"{category}:{key} code identity does not match the "
                            "P2 manifest source identity"
                        )
        else:
            # Legacy tables did not record a builder identity.  Preserve their
            # loading behavior, but never accept a partially declared identity.
            self.table_code_identity_sha256 = None
    ...
    def _expected_table_contract(
        self, category: str, key: str
    ) -> tuple[tuple[int, ...], tuple[int, ...], float]:
        try:
            left, right = key.split("|", 1)
            left_metadata = self.species[left]
            right_metadata = self.species[right]
        except (KeyError, ValueError) as exc:
            raise ValueError(
                f"{category}:{key} is not bound to manifest species"
            ) from exc

        if category == "base":
            left_shells = tuple(int(x) for x in left_metadata["orbital_shells"])
            left_cutoff = float(left_metadata["orbital_cutoff_bohr"])
        elif category == "projector":
            left_shells = tuple(int(x) for x in left_metadata["projector_shells"])
            left_cutoff = float(left_metadata["projector_max_cutoff_bohr"])
        else:
            raise ValueError(f"unknown P2 table category {category!r}")
        right_shells = tuple(int(x) for x in right_metadata["orbital_shells"])
        right_cutoff = float(right_metadata["orbital_cutoff_bohr"])
        expected_support = left_cutoff + right_cutoff
        if not np.isfinite(expected_support) or expected_support <= 0.0:
            raise ValueError(
                f"{category}:{key} has invalid support in manifest species metadata"
            )
        return left_shells, right_shells, expected_support

    def _validate_loaded_table_contract(
        self,
        category: str,
        key: str,
        ...
        expected_left, expected_right, expected_support = (
            self._expected_table_contract(category, key)
        )
        if table.left_shells != expected_left or table.right_shells != expected_right:
            raise ValueError(
                f"{category}:{key} shard shell contract mismatch: "
                f"shard=({list(table.left_shells)}, {list(table.right_shells)}), "
                f"manifest=({list(expected_left)}, {list(expected_right)})"
            )

        support_tolerance = 1.0e-10
        if not np.isfinite(table.support_bohr) or not np.isclose(
            float(table.support_bohr),
            expected_support,
            rtol=1.0e-12,
            atol=support_tolerance,
        ):
            raise ValueError(
                f"{category}:{key} shard support contract mismatch: "
                f"shard={table.support_bohr}, manifest species={expected_support}"
            )

        if "support_bohr" not in metadata:
            if self.table_code_identity_sha256 is not None:
                raise ValueError(
                    f"{category}:{key} hardened shard metadata lacks support_bohr"
                )
            return
        declared_support = float(metadata["support_bohr"])
        if not np.isfinite(declared_support) or not np.isclose(
            declared_support,
            expected_support,
            rtol=1.0e-12,
            atol=support_tolerance,
            ...
            raise ValueError(
                f"{category}:{key} manifest support contract mismatch: "
                f"table metadata={declared_support}, species={expected_support}"
            )
        if not np.isclose(
            declared_support,
            float(table.support_bohr),
            ...
            raise ValueError(
                f"{category}:{key} shard support {table.support_bohr} disagrees "
                f"with table metadata {declared_support}"
            )
    ...
    def _load_table(
        ...
        *,
        ...
        self._validate_loaded_table_contract(category, key, metadata, table)
...
class P2TableAssembler:
    ...
    def _assemble_sparse_blocks(
        ...
        *,
        ...
        assembly_stats: dict[str, Any] = {
            ...
            "projector_overlap_execution": "not_started",
            "projector_overlap_estimated_bytes": 0,
            "projector_overlap_batch_budget_bytes": int(
                self.projector_overlap_cache_max_bytes
            ),
            "projector_overlap_budget_exceeded": False,
            "projector_overlap_fallback_reason": None,
        ...
        try:
            ...
            estimated_overlap_bytes = sum(
                len(entries)
                * int(self.store.species[projector_species]["projector_norb"])
                * int(self.store.species[orbital_species]["orbital_norb"])
                * np.dtype(np.float64).itemsize
                for (projector_species, orbital_species), entries in (
                    overlap_buckets.items()
                )
            )
            assembly_stats["projector_overlap_unique"] = len(overlap_displacements)
            assembly_stats["projector_overlap_estimated_bytes"] = int(
                estimated_overlap_bytes
            )
            assembly_stats["vnl_contractions"] = len(contractions)
            overlap_budget_exceeded = (
                estimated_overlap_bytes > self.projector_overlap_cache_max_bytes
            )
            assembly_stats["projector_overlap_budget_exceeded"] = bool(
                overlap_budget_exceeded
            )

            if overlap_budget_exceeded:
                assembly_stats["projector_overlap_execution"] = (
                    "scalar_lru_fallback"
                )
                assembly_stats["projector_overlap_fallback_reason"] = (
                    "estimated_projector_overlap_bytes_exceed_batch_budget"
                )
                assembly_stats["scalar_fallback_blocks"] = len(keys)

                # The scalar oracle below re-enumerates the requested blocks and
                # routes projector overlaps through the byte-bounded LRU.  Drop
                # the batch gather working set first so it is not retained beside
                # the fallback assembly.
                output.clear()
                base_buckets.clear()
                overlap_displacements.clear()
                overlap_buckets.clear()
                contractions.clear()
                ...
                for key in keys:
                    ...
                    output[key] = self._assemble_block_from_geometry(
                        geometry, context=context
                    )
                assembly_stats["status"] = "success"
                self._last_batch_stats = self._merge_batch_stats(
                    context, assembly_stats
                )
                return output

            assembly_stats["projector_overlap_execution"] = "evaluate_many"
    ...
    def assemble_sparse_blocks(
        ...
        *,
        ...
        gathers required blocks and buckets radial-table queries by shard.  If
        the retained projector-overlap matrices fit the configured byte budget,
        it calls ``RadialBlockTable.evaluate_many`` and scatters the exact AO
        blocks back to their requested keys.  Larger working sets fall back to
        the scalar oracle with the structure-local byte-bounded overlap LRU.
        """
