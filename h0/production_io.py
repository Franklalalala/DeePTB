...
def compare_blocks(pred, ref, counts, scale=1.):
    if not pred and not ref:
        raise ValueError('Cannot validate an empty total block collection')
    if not np.isfinite(scale) or scale <= 0 or any(n <= 0 for n in counts):
        raise ValueError('Invalid comparison scale or orbital counts')
    metrics={}
    for group in ['total','onsite','hopping']:
        ...
        for key in set(pred)|set(ref):
            ...
            if truth.shape!=shape or guess.shape!=shape or not np.isfinite(truth).all() or not np.isfinite(guess).all():
                raise ValueError('Invalid comparison block shape or nonfinite matrix')
            delta=guess-truth
        ...
        if not errors:
            metrics[group] = {'elements':0, 'reference_nonzero_elements':0,
                'mae':None, 'rmse':None, 'max_abs':None,
                'mae_on_reference_nonzero':None, 'reference_rms':None, 'worst':None}
            continue
        ...
        metrics[group]={'elements':len(e),'reference_nonzero_elements':int(np.count_nonzero(active)),
                        ...
                        'mae_on_reference_nonzero':float(np.abs(e[active]).mean()) if np.any(active) else None,
