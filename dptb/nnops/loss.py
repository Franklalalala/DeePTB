...
class Loss:
    ...
    def __new__(cls, method: str, **kwargs):
        from . import band_losses  # registers the optional spectral objectives
...
# Lazy compatibility exports avoid a cycle when band_losses is imported first.
_BAND_EXPORTS = {"EigHamH0ResLoss", "HamilAbsGaugedLoss", "NextHAMKSpaceLoss",
                 "FW10EigLoss", "BandStage2Loss"}


def __getattr__(name):
    if name in _BAND_EXPORTS:
        from . import band_losses
        return getattr(band_losses, name)
