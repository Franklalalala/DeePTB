...
class CudaPeriodicFFTGridAOCache:
    ...
    def contract_pair(self, left, right, center_i, center_j_home, image) -> np.ndarray:
        ...
        result,_ = self._contract_pair_device(left,right,center_i,center_j_home,image)
        return result.cpu().numpy()

    def _contract_pair_device(self, left, right, center_i, center_j_home, image):
        """Validated native contraction; keep the small result on its stream."""
        ...
        if np.linalg.norm(np.asarray(center_i) - center_j) > left.basis.rcut + right.basis.rcut:
            out=torch.zeros((left.norb,right.norb),dtype=torch.float64,device=self.device)
            return out,torch.zeros_like(out) if self.has_spin_z else None
        ...
        res = clg.contract_single_pair_cuda(
            ...
            self.has_spin_z, a["spin_z_potential"] if self.has_spin_z else dummy_spin,
        ...
        return res[0],res[1] if self.has_spin_z else None

    def contract_chunk(self, pairs, atom_species, evaluators, home_positions):
        """Bounded result staging, with shared mean/spin-z AO supports.

        Each pair still uses the same validated native intersection and GEMM.
        CPU transfer is deferred until the caller's finite chunk is complete.
        """
        tensors=[];shapes=[]
        for i,j,R,ci,cj in pairs:
            left,right=evaluators[atom_species[i]],evaluators[atom_species[j]]
            v,z=self._contract_pair_device(left,right,ci,home_positions[j],R)
            tensors.append(v.reshape(-1));shapes.append(v.shape)
            if self.has_spin_z:tensors.append(z.reshape(-1))
        if not tensors:return []
        start=time.perf_counter()
        flat=torch.cat(tensors).cpu().numpy()
        self._stats['result_transfer_seconds']=self._stats.get('result_transfer_seconds',0.)+time.perf_counter()-start
        self._stats['result_transfer_batches']=self._stats.get('result_transfer_batches',0)+1
        self._stats['peak_result_staging_bytes']=max(self._stats.get('peak_result_staging_bytes',0),2*flat.nbytes)
        offset=0;out=[]
        for shape in shapes:
            size=int(np.prod(shape));v=flat[offset:offset+size].reshape(shape).copy();offset+=size
            z=None
            if self.has_spin_z:
                z=flat[offset:offset+size].reshape(shape).copy();offset+=size
            out.append((v,z))
        return out
    ...
    def stats(self) -> dict[str, Any]:
        return {
            **self._stats,
            ...
            "contraction_timing_scope": "native call wall time; chunk result transfer recorded separately",
        }
