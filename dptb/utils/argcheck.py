...
def slem():
    ...
    doc_so2_fusion_mode = "SO2_Linear fusion mode. Supported: `staged`, `streamed_m_major_ref`, `streamed_m_major_aggressive`, `streamed_m_major_cueq`, `streamed_m_major_triton_fused`. Default keeps the current staged path; `streamed_m_major_cueq` uses the grouped m-major SO2 dataflow and should be paired with `mole_linear_mode=cueq_indexed_linear` for the cuEquivariance indexed-linear backend. `streamed_m_major_triton_fused` is an experimental route that fuses streamed SO2 pack/scatter bridge ops with Triton when available and otherwise uses a torch fallback."
