...
from dptb.plugins.monitor import TrainLossMonitor, LearningRateMonitor, Validationer, TensorBoardMonitor, CleanCompatibleTensorBoardMonitor, DeepDoctorMonitor, SO2ModuleMonitor, PreTPBlockMonitor, TrainOnsiteLossMonitor, TrainHoppingLossMonitor, TrainZLossMonitor, ExpertLoadCVMonitor, ScalarFieldMonitor, ParamDynamicsMonitor
...
def train(
    ...
    train_options = jdata["train_options"]
    clean_compatible_tensorboard = bool(
        train_options.get("clean_compatible_tensorboard", False)
    )
    ...
    if flow_enabled:
        for flow_stat_name in flow_scalar_fields:
            if clean_compatible_tensorboard and flow_stat_name.startswith("validation_"):
                continue
    ...
    if validation_datasets and not clean_compatible_tensorboard:
...
##############################
    ...
    if bool(train_options.get("monitor_param_dynamics", False)):
        ...
        if param_dynamics_tb_opt is None:
            param_dynamics_tb = bool(
                train_options.get("use_tensorboard", False)
                and not clean_compatible_tensorboard
            )
        else:
    ...
    if jdata["train_options"]["use_tensorboard"]:
        ...
        if clean_compatible_tensorboard:
            trainer.register_plugin(
                CleanCompatibleTensorBoardMonitor(
                    interval=[(1, 'iteration'), (1, 'epoch')],
    ...
    if clean_compatible_tensorboard:
        log_field = [
            field for field in log_field
            if not field.startswith("validation_")
        ]
    ...
    if output:
        ...
        trainer.register_plugin(Saver(
            interval=[(jdata["train_options"].get("save_freq"), 'iteration')] if jdata["train_options"].get(
