...
def key_for(table):
    import json
    from .numerical_identity import capture
    h=hashlib.sha256(b'nacf-prepared/v2')
    for name in ('radial.py','prepared.py','numerical_identity.py'):
        h.update(Path(__file__).with_name(name).read_bytes())
    h.update(json.dumps(capture(('numpy','scipy','torch'))['identity'],sort_keys=True).encode())
...
def cached_table(source,cache_dir,*,device,dtype,backend):
    ...
    if path.exists():
        ...
        for name,value in record['buffers'].items():
            ...
            obj.register_buffer(name,value)
    ...
    if any(not torch.isfinite(v).all() for v in obj.buffers()):raise ValueError('Nonfinite prepared radial buffer after dtype conversion')
    obj.prepared_cache='prepared';return obj
