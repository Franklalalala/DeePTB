...
from infer_nacf_geometry import load_predictor
...
def benchmark(predictor, atoms, repeats=5):
    ...
    functions = dict(cpu_prior_and_s=cpu_prior,gpu_prior_and_s=prepared.plans[0],
...
def main():
    ...
    report=dict(torch_version=torch.__version__,device=str(predictor.device),
                ...
                timing_scope=__doc__,cases=[])
    for atoms in read(args.geometry,index=':'):
        row=benchmark(predictor,atoms,args.repeats)
