...
"""Apply the Triton exact graph-persistent V3 route hook.

Run this script from the DeePTB repository root after applying the V3 additive
...
MARKER = "# Triton exact graph-persistent V3 overlay"
...
COMPLEX_HOOK = '''    if _use_complex_exact_gp_v3():
...
def insert_import_block(text: str) -> str:
    if MARKER in text:
        return text
    # Prefer to place V3 before the V2 overlay so V3 can win the dispatch when both are enabled.
    v2_marker = "# Triton exact graph-persistent V2 overlay\n"
    if v2_marker in text:
        return text.replace(v2_marker, IMPORT_BLOCK + "\n" + v2_marker, 1)
...
def insert_complex_hook(text: str) -> str:
    if "_complex_exact_moe_linear_v3(" in text:
        return text
    pattern = re.compile(
        r"(def grouped_complex_exact_moe_linear\([\s\S]*?\) -> torch\.Tensor:\n"
        r"\s+split_sizes = _canonical_split_sizes\(split_sizes\)\n)"
        r"(\s+if _use_complex_exact_gp_v2\(\):|\s+return _GroupedComplexExactMoELinearFn\.apply\()",
        re.MULTILINE,
    )
    match = pattern.search(text)
    if not match:
