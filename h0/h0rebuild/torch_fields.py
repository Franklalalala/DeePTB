...
def build_periodic_field_torch(structure,species_data,*,ecutrho_ry,fft_shape,
    ...
    with t.no_grad():
        ...
        if xc_upper in {"LDA","LDA_PZ81","PZ81"}:
            ...
            # ABACUS evaluates scalar PBE at abs(rho), including negative
            # Fourier ringing. Keep the signed density and its original
            # gradient; builtin_pbe.scalar owns the absolute-value gates.
            vxc=pbe_torch_hybrid(rho_raw,grad,gvec,density_threshold=pbe_density_threshold,pw_mask=mask)
