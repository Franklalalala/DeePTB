...
The fused path (``_forward_fused``, an OPT-IN performance forward inside the
certified eager fp32/fp64 domain, selected via ``cg_head_impl="fused"``;
``_forward_legacy`` is the P1-3 COMPATIBILITY-DEFAULT and rollback/oracle, and the
inert ``DPTB_FUSED_CG_HEAD`` name is NOT read -- the opt-in is the config field, not
an env var) collapses the legacy per-path Python loop into ``G``
...
Cases (design A.5 / A.8):
  ...
  7. dispatch (P1-3): default (no config, no env) => legacy; ``cg_head_impl="fused"``
     (no env) => fused; ``DPTB_LEGACY_CG_HEAD=1`` overrides a fused config back to
     legacy; ``DPTB_FUSED_CG_HEAD`` is inert (never read -- config only);
  ...
  9. certified-domain routing guard (design A.8): forward() falls back to the legacy
     ...
     even when the head is configured with ``cg_head_impl="fused"`` (precedence
     (b) > (c); this is also the "deterministic overrides a fused config" P1-3
     dispatch unit test, see ``test_deterministic_mode_routes_to_legacy_cpu``).
"""
...
def _build_head(
    ...
    *,
    ...
    ``cg_head_impl`` defaults to ``"legacy"``, matching the module's own P1-3
    COMPATIBILITY-DEFAULT; pass ``"fused"`` explicitly to build a head configured
    for the opt-in performance path."""
...
def fused_opt_in(monkeypatch):
    ...
    Since P1-3, the fused path is reached only via the ``cg_head_impl="fused"``
    constructor/config opt-in -- never via an environment variable.  This fixture
    sets ``DPTB_FUSED_CG_HEAD=1`` -- a reserved name ``forward`` does NOT read (see
    the module comment) -- to assert it never disturbs dispatch either way; tests
    using it build a ``cg_head_impl="fused"`` head and then prove a certified-domain
    guard (or ``DPTB_LEGACY_CG_HEAD``) still routes it to legacy.  Returns
    ``monkeypatch`` for that toggling.
    """
...
# ===========================================================================
# A.5 #7 -- dispatch: default / config opt-in / env rollback (P1-3)
#
# P1-3 flipped the default from fused to legacy and made fused reachable only via
# the ``cg_head_impl`` constructor/config field (never an env var).  The four
# cases below are the dispatch unit tests required by that fix:
#   1. default (no config, no env)            -> legacy
#   2. cg_head_impl="fused" (no env)           -> fused
#   3. DPTB_LEGACY_CG_HEAD=1 overrides a fused config -> legacy
#   4. deterministic algorithms override a fused config -> legacy (A.8 section,
#      see ``test_deterministic_mode_routes_to_legacy_cpu``)
# ===========================================================================
def test_default_dispatch_is_legacy(monkeypatch):
    """P1-3 COMPATIBILITY-DEFAULT: with no ``cg_head_impl`` opt-in and no env
    override, forward() routes to the legacy per-path loop (precedence (d)) --
    bit-stable with pre-fusion (0715-refactor) checkpoints/configs.
    ``DPTB_FUSED_CG_HEAD=1`` remains inert (not read; opting in requires the config
    field, not this env var)."""
    ...
    assert head.cg_head_impl == "legacy"
    x = torch.randn(5, head.irreps_in.dim, dtype=torch.float64)
...
def test_config_fused_selects_fused_path(monkeypatch):
    """``cg_head_impl="fused"`` alone (no env vars at all) selects the fused
    grouped-einsum path inside the certified eager fp32/fp64 domain (precedence
    (c))."""
    monkeypatch.delenv(_LEGACY_CG_HEAD_ENV, raising=False)
    monkeypatch.delenv(_FUSED_CG_HEAD_ENV, raising=False)
    head = _build_head(
        WATER_BASIS, symmetrize=True, dtype=torch.float64, seed=23, cg_head_impl="fused"
    )
    assert head.cg_head_impl == "fused"
    x = torch.randn(5, head.irreps_in.dim, dtype=torch.float64)
    assert torch.equal(head(x), head._forward_fused(x))


@pytest.mark.parametrize("basis", [WATER_BASIS, CRYSTAL_BASIS])
def test_env_var_rollback_overrides_fused_config(basis, monkeypatch):
    """``DPTB_LEGACY_CG_HEAD=1`` overrides an explicit ``cg_head_impl="fused"``
    config back to the legacy loop (precedence (a) > (c)); removing the override
    lets the fused config take effect again (the inert ``DPTB_FUSED_CG_HEAD``
    affirmation changes neither outcome)."""
    head = _build_head(
        basis, symmetrize=True, dtype=torch.float64, seed=8, cg_head_impl="fused"
    )
    x = torch.randn(5, head.irreps_in.dim, dtype=torch.float64)
    ...
    dispatched = head(x)
    assert torch.equal(dispatched, head._forward_legacy(x))  # env override wins

    monkeypatch.delenv(_LEGACY_CG_HEAD_ENV, raising=False)
    fused = head(x)
    assert torch.equal(fused, head._forward_fused(x))  # falls through to config

    # The two paths still agree within the certified fp64 tolerance.
    assert _max_abs(dispatched, fused) <= FWD_ATOL[torch.float64]


def test_env_var_non_one_values_leave_fused_config_selected(monkeypatch):
    """Only the literal string "1" activates the legacy override: non-"1" values of
    ``DPTB_LEGACY_CG_HEAD`` leave an explicit ``cg_head_impl="fused"`` config
    selected."""
    head = _build_head(
        WATER_BASIS, symmetrize=False, dtype=torch.float64, seed=8, cg_head_impl="fused"
    )
    x = torch.randn(3, head.irreps_in.dim, dtype=torch.float64)
    monkeypatch.setenv(_FUSED_CG_HEAD_ENV, "1")  # inert affirmation
    for value in ("0", "true", "yes", ""):
        monkeypatch.setenv(_LEGACY_CG_HEAD_ENV, value)
        assert torch.equal(head(x), head._forward_fused(x))


def test_legacy_env_overrides_fused_config(monkeypatch):
    """Precedence (a) > (c): ``DPTB_LEGACY_CG_HEAD=1`` forces the legacy loop even
    when the head is explicitly configured with ``cg_head_impl="fused"`` (and the
    inert ``DPTB_FUSED_CG_HEAD=1`` affirmation is also set)."""
    head = _build_head(
        WATER_BASIS, symmetrize=True, dtype=torch.float64, seed=22, cg_head_impl="fused"
    )
...
def test_invalid_cg_head_impl_rejected():
    """An unrecognized ``cg_head_impl`` value fails fast at construction, not
    silently at forward-dispatch time."""
    with pytest.raises(ValueError):
        _build_head(
            WATER_BASIS, symmetrize=True, dtype=torch.float64, cg_head_impl="turbo"
        )
...
#
# The fused path is reachable ONLY inside the certified domain: eager fp32/fp64.
# Outside it -- autocast, non-fp32/64 input dtype, or
# ``use_deterministic_algorithms(True)`` -- forward() must transparently route to the
# bit-compatible legacy loop, REGARDLESS of ``cg_head_impl`` (precedence (b) > (c)).
# Every head built below is explicitly configured with ``cg_head_impl="fused"`` so
# each routing test proves the guard overrides a genuine fused selection via
# ``torch.equal`` to ``_forward_legacy`` in the SAME context: if the guard did NOT
# fire, forward() would run the fused path, which differs from legacy by
# reassociation roundoff, so bit-equality can only mean the guard fired (these are
# routing proofs, NOT numeric-parity claims -- low precision is exactly where fused
# and legacy are ALLOWED to diverge).  ``test_deterministic_mode_routes_to_legacy_cpu``
# is the dispatch unit test for "deterministic overrides a fused config" (P1-3 item
# 4).  The ``fused_opt_in`` fixture also sets the inert ``DPTB_FUSED_CG_HEAD`` to
# assert that reserved name never disturbs the guard either.
# ===========================================================================
def test_cpu_autocast_bf16_routes_to_legacy(fused_opt_in):
    ...
    bit-for-bit (both run the legacy loop under the same autocast), even though the
    head is explicitly configured with ``cg_head_impl="fused"``."""
    head = _build_head(
        WATER_BASIS, symmetrize=True, dtype=torch.float32, seed=15, cg_head_impl="fused"
    )
...
def test_deterministic_mode_routes_to_legacy_cpu(fused_opt_in):
    """P1-3 dispatch unit test: deterministic algorithms override a fused config.
    Under ``use_deterministic_algorithms(True)`` forward() routes to the per-path
    loop (which has no duplicate-destination scatter) and equals it exactly, even
    though the head is explicitly configured with ``cg_head_impl="fused"``
    (precedence (b) > (c)).  try/finally always restores the global flag."""
    head = _build_head(
        WATER_BASIS, symmetrize=True, dtype=torch.float64, seed=14, cg_head_impl="fused"
    )
    x = torch.randn(5, head.irreps_in.dim, dtype=torch.float64)
...
def test_half_input_routes_to_legacy(fused_opt_in):
    ...
    routes to the legacy loop even when ``cg_head_impl="fused"``.
    ...
    head32 = _build_head(
        WATER_BASIS, symmetrize=True, dtype=torch.float32, seed=13, cg_head_impl="fused"
    )
    ...
    head16 = _build_head(
        WATER_BASIS, symmetrize=True, dtype=torch.float16, seed=13, cg_head_impl="fused"
    )
...
def test_cuda_deterministic_mode_routes_to_legacy(fused_opt_in):
    ...
    legacy loop and equals it, with NO exception, even though the head is
    explicitly configured with ``cg_head_impl="fused"``.  Both paths share cuBLAS
    matmuls, so CUBLAS_WORKSPACE_CONFIG (set at import) is what lets the assertion
    run at all; it is the routing -- proven by ``torch.equal`` -- that this
    certifies (fused would differ by roundoff)."""
    head = _build_head(
        WATER_BASIS, symmetrize=True, dtype=torch.float32, seed=18, cg_head_impl="fused"
    ).cuda()
...
def test_cuda_autocast_fp16_routes_to_legacy(fused_opt_in):
    ...
    bit-for-bit -- proves routing under the CUDA/device-agnostic autocast flag, even
    though the head is explicitly configured with ``cg_head_impl="fused"``."""
    head = _build_head(
        WATER_BASIS, symmetrize=True, dtype=torch.float32, seed=19, cg_head_impl="fused"
    ).cuda()
