...
@Embedding.register("emoles_openequi_eqv3_swiglu_s2_ffn")
class EMolESOpenequiEqV3SwiGLUS2FFN(EMolESOpenequiEqV3FFN):
    """
    Preset for merged RMS norm, hidden SwiGLU-S2 activation, and EqV3 node FFN.
    """

    def __init__(self, **kwargs):
        kwargs = dict(kwargs)
        kwargs["equivariant_norm_type"] = "merged_rms"
        kwargs["hidden_edge_activation_type"] = "swiglu_s2"
        kwargs.setdefault("swiglu_s2_grid_resolution", [14, 14])
        if float(kwargs.get("ffn_hidden_factor", 0.0)) <= 1.0:
            kwargs["ffn_hidden_factor"] = 4.0
        super().__init__(**kwargs)
