...
class HamiltonianCFM:
    ...
    def prepare_batch(
        ...
        *,
        ...
        if self.block_ode:
            # Route-specific one-step assembly lives on the per-route adapter
            # (block_ode/route_adapters.py).  The three routes are mutually
            # exclusive and, once block_ode is set, exhaustive; ``select`` mirrors
            # the historical branch order (uureal, then residual, then generic
