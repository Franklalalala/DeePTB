...
std::vector<torch::Tensor> contract_single_pair_cuda(
    ...
    TORCH_CHECK(device.is_cuda(), "pair contraction requires CUDA");
    const c10::cuda::CUDAGuard guard(device);
    using namespace local_check;
    anchor(ga_lo,ga_hi,ga_lookup,ga_values,device,norb_i);
    anchor(gb_lo,gb_hi,gb_lookup,gb_values,device,norb_j);
    vector3(shift,device,torch::kInt64);coordinates(shift);
    tensor(ga_potential,device,torch::kFloat64,1);finite(ga_potential);
    TORCH_CHECK(ga_potential.numel()==ga_values.size(0), "potential length mismatch");
    tensor(ga_spin_z_potential,device,torch::kFloat64,1);
    TORCH_CHECK(ga_spin_z_potential.numel()==(has_spin_z?ga_values.size(0):0), "spin potential length mismatch");
    if(has_spin_z) finite(ga_spin_z_potential);
    TORCH_CHECK(std::isfinite(grid_weight) && grid_weight>0, "invalid grid weight");
    product(norb_i,norb_j);
    auto opt_f64 = torch::dtype(torch::kFloat64).device(device);
    if(!ga_lookup.numel() || !gb_lookup.numel())
        return {torch::zeros({norb_i,norb_j},opt_f64),has_spin_z?torch::zeros({norb_i,norb_j},opt_f64):torch::empty({0},opt_f64)};
...
std::vector<std::vector<torch::Tensor>> contract_pairs_batch_cuda(
    ...
    TORCH_CHECK(device.is_cuda(), "batch contraction requires CUDA");
    const c10::cuda::CUDAGuard guard(device);
    using namespace local_check;
    tensor(pair_i,device,torch::kInt32,1);tensor(pair_j,device,torch::kInt32,1);
    tensor(pair_R,device,torch::kInt64,2);vector3(grid_shape,device,torch::kInt64);
    TORCH_CHECK(pair_j.numel()==pair_i.numel() && pair_R.size(0)==pair_i.numel() && pair_R.size(1)==3, "pair shape mismatch");
    auto count=anchors_values.size();
    TORCH_CHECK(anchors_lo.size()==count && anchors_hi.size()==count && anchors_lookup.size()==count &&
        anchors_potential.size()==count && atom_norbs.size()==count &&
        (has_spin_z?anchors_spin_z_potential.size()==count:anchors_spin_z_potential.empty()), "anchor list length mismatch");
    TORCH_CHECK(std::isfinite(grid_weight) && grid_weight>0 && (grid_shape>0).all().item<bool>() &&
        (grid_shape<=coordinate_limit).all().item<bool>(), "invalid batch grid");
    if(pair_i.numel()) TORCH_CHECK(pair_i.min().item<int>()>=0 && pair_j.min().item<int>()>=0 &&
        pair_i.max().item<int64_t>()<int64_t(count) && pair_j.max().item<int64_t>()<int64_t(count), "pair index out of range");
    auto bound=torch::floor_divide(torch::full_like(grid_shape,coordinate_limit),grid_shape);
    TORCH_CHECK(((pair_R>=-bound)&(pair_R<=bound)).all().item<bool>(), "pair shift exceeds supported range");
    for(size_t k=0;k<count;++k) {
        anchor(anchors_lo[k],anchors_hi[k],anchors_lookup[k],anchors_values[k],device,atom_norbs[k]);
        tensor(anchors_potential[k],device,torch::kFloat64,1);finite(anchors_potential[k]);
        TORCH_CHECK(anchors_potential[k].numel()==anchors_values[k].size(0), "potential length mismatch");
        if(has_spin_z) {
            tensor(anchors_spin_z_potential[k],device,torch::kFloat64,1);finite(anchors_spin_z_potential[k]);
            TORCH_CHECK(anchors_spin_z_potential[k].numel()==anchors_values[k].size(0), "spin potential length mismatch");
        }
    }
    int M = pair_i.size(0);
    auto opt_f64 = torch::dtype(torch::kFloat64).device(device);
