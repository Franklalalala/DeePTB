...
def HybridMuon():
    ...
    return [
        ...
        Argument("expert_update_scale", str, optional=True, default="none",
                 extra_check=lambda v: v in {"none", "sqrt_load", "const"},
                 extra_check_errmsg="expert_update_scale must be none, sqrt_load or const",
...
def _edge_router_arguments():
    ...
    return [
        ...
        Argument("edge_router_logit", str, optional=True, default="raw",
                 ...
                 doc="Router logits: `raw` (production) or `cosine` = edge_router_logit_scale * cos(hidden, expert row), bounded so the sigmoid cannot saturate to 1.0."),
        Argument("edge_router_logit_scale", float, optional=True, default=10.0, doc="Scale of the cosine router logits. Default: 10."),
        Argument("edge_router_select", str, optional=True, default="sigmoid",
                 extra_check=lambda v: v in {"sigmoid", "logit"}, extra_check_errmsg="edge_router_select must be sigmoid or logit",
                 doc="Top-k selection score: `sigmoid` of the logits (production) or the logits themselves."),
        Argument("edge_router_bias_at_eval", bool, optional=True, default=False, doc="Keep the frozen load-balancing bias in the eval selection (train and eval then select alike)."),
        Argument("edge_router_bias_speed", float, optional=True, default=0.005, doc="Sign-rule step of the load-balancing bias per optimizer step. Default: 0.005."),
        Argument("edge_router_bias_schedule", str, optional=True, default="const",
                 extra_check=lambda v: v in {"const", "follow_lr", "freeze_decay"},
                 extra_check_errmsg="edge_router_bias_schedule must be const, follow_lr or freeze_decay",
...
def slem_edge():
    return slem() + _edge_router_arguments()


def slem_edge_h0():
    return slem_h0() + _edge_router_arguments()
