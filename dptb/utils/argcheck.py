...
def slem():
    ...
    doc_mole_linear_mode = "MoLELinear backend. Supported: `split_loop`, `indexed_ref`, `cueq_indexed_linear`, `triton_grouped_linear`. Default `None` uses the DPTB_MOLE_LINEAR_MODE environment variable or falls back to `split_loop`. `triton_grouped_linear` uses a Triton persistent grouped GEMM backend and falls back to split-loop formulas when Triton is unavailable unless DPTB_TRITON_LINEAR_REQUIRE=1."
    doc_mole_linear_m0_mode = "Optional override for the scalar m=0 MoLELinear backend inside SO2_Linear. Supported: `split_loop`, `indexed_ref`, `cueq_indexed_linear`, `triton_grouped_linear`. Default `None` inherits `mole_linear_mode` or uses the DPTB_MOLE_LINEAR_M0_MODE environment variable."
    doc_so2_m_linear_mode = "SO2_m_Linear backend for m>0. Supported: `standard`, `cueq_complex_indexed_linear`, `cueq_segmented_complex_indexed_linear`, `triton_complex_grouped_linear`, `triton_complex_moe_fused_linear`. The cuEq complex modes fuse the indexed MoLE linear and real/imag SO2 post-processing through cuEquivariance indexed-linear calls and require `mole_linear_mode=cueq_indexed_linear`. `triton_complex_grouped_linear` is an experimental opt-in backend that applies the m>0 complex grouped formula through the Triton grouped-linear module when available, otherwise falling back to the standard path unless DPTB_TRITON_LINEAR_REQUIRE=1. `triton_complex_moe_fused_linear` is a more aggressive experimental backend that fuses full-expert coefficient mixing with m>0 complex linear; it currently supports bias=False and num_shared_experts=0 only."
