    ...
    args = [
        ...
        Argument("prior_node", str, optional=True, default="",
                 doc="Split prior: family for the node/onsite prior (basis_onsite / "
                     "overlap_huckel / external). Must be set together with prior_edge."),
...
def train_options():
    ...
    doc_save_freq = (
        "Checkpoint save frequency in committed optimizer steps. "
        "Default: `10`."
    )
...
def _lmdb_dataset_type():
    return Argument(
        "type",
        str,
        optional=True,
        default="LMDBDataset",
        extra_check=lambda value: value == "LMDBDataset",
        extra_check_errmsg=(
            "0726-light supports only data_options.*.type='LMDBDataset'."
        ),
        doc="The maintained dataset backend. Only LMDBDataset is supported.",
    )


def _lmdb_data_fields():
    """Shared strict schema for maintained LMDB dataset splits."""
    return [
        _lmdb_dataset_type(),
        Argument("root", str, optional=False, doc="Root containing LMDB shard directories."),
        Argument("prefix", str, optional=False, doc="Shard-directory prefix."),
        Argument("separator", str, optional=True, default=".", doc="Prefix/suffix separator."),
        Argument("get_Hamiltonian", bool, optional=True, default=False, doc="Load Hamiltonian blocks."),
        Argument("get_H0", bool, optional=True, default=False, doc="Load physical H0 initialization data."),
        Argument("get_P2", bool, optional=True, default=False, doc="Backward-compatible switch for the selected P2/P23 prior."),
        Argument("prior_kind", str, optional=True, default="p2", doc="Selected physical prior: p2 or p23."),
        Argument("residual_hamiltonian", bool, optional=True, default=False, doc="Train on dH = H - H0."),
        Argument("residual_shrink_policy", str, optional=True, default="error", doc="Residual shrink gate: error, warn, or off."),
        Argument("min_residual_shrink", [float, int], optional=True, default=1.2, doc="Minimum residual shrink ratio."),
        Argument("h0_key", str, optional=True, default="hamiltonian_0", doc="Raw LMDB H0 key."),
        Argument("prefer_precomputed_h0", bool, optional=True, default=True, doc="Prefer stored node_h0/edge_h0 features."),
        Argument("p2_key", str, optional=True, default="", doc="Deprecated explicit raw prior key; normally derived from prior_kind."),
        Argument("prefer_precomputed_p2", bool, optional=True, default=True, doc="Prefer stored selected-prior RME features."),
        Argument("require_full_h_target", bool, optional=True, default=False, doc="Require versioned absolute Full-H target provenance."),
        Argument("require_residual_h_target", bool, optional=True, default=False, doc="Require versioned residual-H target provenance."),
        Argument("require_uureal_block_ode", bool, optional=True, default=False, doc="Require compact uu_real block-ODE records."),
        Argument("require_residual_from_full_h_target", bool, optional=True, default=False, doc="Require online H-H0 materialization from absolute Full H."),
        Argument("expected_p2_source_fingerprint", str, optional=True, default="", doc="Expected selected-prior source SHA256."),
        Argument("expected_physical_h0_source_fingerprint", str, optional=True, default="", doc="Expected physical-H0 source SHA256."),
        Argument("allow_unbound_prior_source_fingerprint", bool, optional=True, default=False, doc="Development-only prior provenance escape hatch."),
        Argument("audit_p2_representations", bool, optional=True, default=False, doc="Audit selected-prior RME/AO consistency at ingest."),
        Argument("require_p2_blocks", bool, optional=True, default=False, doc="Require selected-prior AO blocks."),
        Argument("get_overlap", bool, optional=True, default=False, doc="Load overlap blocks."),
        Argument("get_DM", bool, optional=True, default=False, doc="Load density matrices."),
        Argument("get_eigenvalues", bool, optional=True, default=False, doc="Load eigenvalues and k-points."),
    ]
...
def train_data_sub():
    return Argument(
        ...
        dict,
        optional=False,
        sub_fields=_lmdb_data_fields(),
        sub_variants=[],
        doc="LMDB dataset settings for training.",
    )
...
def validation_data_sub():
    return Argument(
        ...
        sub_fields=_lmdb_data_fields(),
        sub_variants=[],
        doc="LMDB dataset settings for validation.",
    )
...
def reference_data_sub():
    return Argument(
        ...
        sub_fields=_lmdb_data_fields(),
        sub_variants=[],
        doc="LMDB dataset settings for reference batches.",
    )
...
def test_data_sub():
    return Argument(
        ...
        sub_fields=_lmdb_data_fields(),
        sub_variants=[],
        doc="LMDB dataset settings for testing.",
    )
...
def embedding():
    ...
    return Variant("method", [
        ...
        ],optional=True, default_tag="lem_moe_v3", doc=doc_method)
...
def prediction():
    doc_method = "The maintained prediction model: e3tb or block_native."
...
def model_options():
    ...
    doc_model_options = "The parameters for the maintained embedding-plus-prediction model."
...
def run_options():
    doc_task = "The maintained postprocessing task: band or write_block."
...
def AtomicData_options_sub():
    ...
    doc_er_max = (
        "Optional environment-edge cutoff used by maintained embeddings with "
        "a distinct environment graph."
    )
    doc_oer_max = (
        "Optional onsite-environment cutoff used by maintained embeddings "
        "with a distinct onsite graph."
    )
...
def get_cutoffs_from_model_options(model_options):
    """Extract cutoffs for the maintained embedding model family."""
    supported = {
        "emoles",
        "emoles_openequi",
    ...
    if not isinstance(embedding, dict):
        raise ValueError("model_options.embedding is required in 0726-light.")
    method = embedding.get("method")
    if method not in supported:
        raise ValueError(f"Unsupported embedding method in 0726-light: {method!r}.")
    if "r_max" not in embedding:
        raise ValueError(f"embedding.r_max is required for method {method!r}.")
    return embedding["r_max"], None, None

def collect_cutoffs(jdata):
    """Collect cutoffs for the maintained embedding model family."""
    r_max, er_max, oer_max = get_cutoffs_from_model_options(
        jdata["model_options"]
    )
    cutoff_options = {
        "r_max": r_max,
