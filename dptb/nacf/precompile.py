...
def main():
    ...
    a=p.parse_args();archs=a.arch or ['8.0','8.6','8.9','9.0']
    native=[x.removesuffix('+PTX') for x in archs];ptx=[x.removesuffix('+PTX') for x in archs if x.endswith('+PTX')]
    try:
        m=verify()
        same_targets=a.arch is None or (m['architectures']==native and m.get('ptx_architectures',[])==ptx)
        if not a.force and same_targets: print('REUSED',m['sha256']);return
